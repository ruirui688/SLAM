...
## 2026-05-11 P216 — No-manual training redirect

- **Goal:** Audit whether the P206-P215 no-label evidence-governance path drifted away from real model training, and redirect toward a scientifically valid no-manual or low-manual training target without fabricating persistent-map admission labels.
- **Result: REDIRECT READY, P195 still BLOCKED.** Added a no-manual-training redirect audit and local smoke-prep script. The finding is explicit: P206-P215 are useful evidence governance and paper/export readiness artifacts, but they are not a training target and cannot support learned admission-control claims.
- **Why manual labels were requested:** P195 admission-control learning is correctly blocked because persistent-map `human_admit_label` and cross-session `human_same_object_label` remain independent-label requirements. Existing P193 `target_admit`, `current_weak_label`, `selection_v5`, proxy fields, and model predictions are not ground truth and remain prohibited as training labels.
- **Recommended next route:** P216A dataset-mask-supervised semantic segmentation / binary dynamic-mask front-end training from local TorWIC `AnnotatedSemanticSet_Finetuning.zip` masks and `raw_labels_2d` categories. This can honestly claim semantic segmentation or dynamic-object mask training for SLAM preprocessing, not learned map admission.
- **Local trainability evidence:** AnnotatedSemanticSet has `237` raw label JSON files, `237` paired source-image/indexed-mask samples, and `15` semantic categories with positive pixels. Sequence zips expose `32,743` left RGB/greyscale mask pairs for front-end masking/evaluation. CUDA smoke in `tram` passed on NVIDIA GeForce RTX 3060 with PyTorch `2.4.0+cu118`.
- **Claim boundary:** Allowed claims are dataset-mask-supervised semantic/dynamic mask learning and SLAM front-end raw-vs-masked evaluation. Blocked claims remain learned persistent-map admission and cross-session same-object association until P195 gets independent labels.
- **Outputs:** `tools/prepare_no_manual_training_redirect_p216.py`, `paper/evidence/no_manual_training_redirect_p216.json`, `paper/export/no_manual_training_redirect_p216.md`.
- **Verification:** `python3 tools/prepare_no_manual_training_redirect_p216.py`; `LD_LIBRARY_PATH=/home/rui/miniconda3/envs/tram/lib:$LD_LIBRARY_PATH /home/rui/miniconda3/bin/conda run -n tram python tools/prepare_no_manual_training_redirect_p216.py --cuda-smoke` returns `cuda_smoke=pass`; `python3 -m py_compile tools/prepare_no_manual_training_redirect_p216.py tools/prepare_independent_supervision_p195.py`; `python3 tools/prepare_independent_supervision_p195.py` remains `BLOCKED` with `32/32` blank and `0/32` valid `human_admit_label`, plus `160/160` blank and `0/160` valid `human_same_object_label`.
- **P217 recommendation:** Implement the actual CUDA `TorchDataset`/training baseline for semantic segmentation and binary dynamic masks, with leave-root/frame splits, class weights, mIoU/dynamic-IoU metrics, and exported masks for DROID/ORB front-end evaluation. Keep P195 blocked.

## 2026-05-11 P217 — No-manual dynamic-mask dataset

- **Goal:** Build a reproducible no-manual binary dynamic/non-static mask training dataset from dataset-provided TorWIC `AnnotatedSemanticSet_Finetuning.zip` semantic labels and indexed masks, without fabricating admission labels or using weak/proxy admission fields.
- **Result: READY_NO_MANUAL_DYNAMIC_MASK_DATASET, P195 still BLOCKED.** Added a deterministic builder that reads `source_image.png`, `combined_indexedImage.png`, and `raw_labels_2d.json` members from the local zip, then writes derived RGB copies and binary dynamic masks under `outputs/dynamic_mask_dataset_p217/` without modifying raw data.
- **Target policy:** positive pixels are dataset semantic indices for `cart_pallet_jack`, `fork_truck`, `goods_material`, `misc_dynamic_feature`, `misc_non_static_feature`, `person`, and `pylon_cone`; all static object, context, background, and other indexed pixels are binary `0`. `goods_material` is included only as movable clutter for SLAM front-end masking, not as map-admission ground truth.
- **Dataset:** `237` rows from `79` frame groups. Split is deterministic by frame directory to avoid image-view leakage: train `156` rows / `52` frame groups, val `51` / `17`, test `30` / `10`; frame overlap across splits is `0`. Overall dynamic/non-static positive pixel rate is `0.374176`; split rates are test `0.315705`, train `0.387909`, val `0.366562`.
- **Outputs:** `tools/build_dynamic_mask_dataset_p217.py`, `paper/evidence/dynamic_mask_dataset_p217.csv`, `paper/evidence/dynamic_mask_dataset_p217.json`, `paper/export/dynamic_mask_dataset_p217.md`, plus ignored derived local files under `outputs/dynamic_mask_dataset_p217/` including a sanity contact sheet.
- **Verification:** `python3 tools/build_dynamic_mask_dataset_p217.py` returns `READY_NO_MANUAL_DYNAMIC_MASK_DATASET`; `python3 -m py_compile tools/build_dynamic_mask_dataset_p217.py`; sampled images/masks are readable with matching dimensions; manifest has no `target_admit`, `current_weak_label`, `selection_v5`, model-prediction, or human-label columns; `python3 tools/prepare_independent_supervision_p195.py` remains `BLOCKED` with `32/32` blank and `0/32` valid `human_admit_label`, plus `160/160` blank and `0/160` valid `human_same_object_label`.
- **P218 recommendation:** Add a CUDA `TorchDataset` and compact segmentation/binary-mask training smoke in `tram`, reporting dynamic IoU, mIoU over selected classes if used, class weighting, and exported predicted masks for SLAM front-end raw-vs-masked evaluation. Do not claim learned admission control.

## 2026-05-11 P218 — Dynamic-mask CUDA training smoke

- **Goal:** Train a compact binary dynamic/non-static segmentation smoke from the P217 manifest, report segmentation metrics, and export a few prediction artifacts for later SLAM front-end raw-vs-masked evaluation.
- **Result: P218_DYNAMIC_MASK_TRAINING_SMOKE_COMPLETE, P195 still BLOCKED.** Added `tools/train_dynamic_mask_p218.py`, a small P217-only TorchDataset plus compact UNet-style encoder/decoder. It reads P217 `image_path` and `binary_mask_path`, resizes to `320x180`, uses deterministic settings, and keeps horizontal augmentation off by default.
- **Training smoke:** ran in the README `tram` CUDA environment on NVIDIA GeForce RTX 3060 for `5` epochs, batch size `8`, `BCEWithLogitsLoss(pos_weight=1.489919) + 0.5 * DiceLoss`, selected threshold `0.40` by validation F1. Validation metrics: pixel accuracy `0.826746`, precision `0.716614`, recall `0.913920`, F1 `0.803329`, dynamic IoU `0.671304`, balanced accuracy `0.842796`. Test metrics: pixel accuracy `0.774521`, precision `0.601953`, recall `0.937109`, F1 `0.733038`, dynamic IoU `0.578580`, balanced accuracy `0.815713`.
- **Baselines:** all-background dynamic IoU/F1 is `0.0/0.0` on val and test. The train-positive-prior threshold baseline uses train prior `0.387909`, predicts all background at threshold `0.5`, and is therefore equivalent to all-background.
- **Prediction artifacts:** local ignored files were exported under `outputs/dynamic_mask_training_p218/predictions/`, including prediction masks, probability PNGs, and RGB overlays for three val and three test samples. Paths are listed in `paper/evidence/dynamic_mask_training_p218.json` and `paper/export/dynamic_mask_training_p218.md`; image blobs remain untracked by repository convention.
- **Scientific boundary:** P218 trains dataset-provided semantic dynamic/non-static masks for front-end masking only. It does not use `target_admit`, `current_weak_label`, `selection_v5`, model predictions, or human-label placeholders as labels, and it does not train or claim learned persistent-map admission control.
- **Outputs:** `tools/train_dynamic_mask_p218.py`, `paper/evidence/dynamic_mask_training_p218.json`, `paper/export/dynamic_mask_training_p218.md`, plus ignored local prediction artifacts under `outputs/dynamic_mask_training_p218/`.
- **Verification:** `python3 -m py_compile tools/train_dynamic_mask_p218.py tools/prepare_independent_supervision_p195.py`; `LD_LIBRARY_PATH=/home/rui/miniconda3/envs/tram/lib:$LD_LIBRARY_PATH /home/rui/miniconda3/bin/conda run -n tram python tools/train_dynamic_mask_p218.py --epochs 5 --batch-size 8 --resize-width 320 --resize-height 180`; `python3 tools/prepare_independent_supervision_p195.py` remains `BLOCKED` with `32/32` blank and `0/32` valid `human_admit_label`, plus `160/160` blank and `0/160` valid `human_same_object_label`.
- **P219 recommendation:** Use the exported P218 masks as a bounded front-end masking input for raw-vs-masked SLAM evaluation on held-out/local sequences, starting with a small DROID/ORB smoke and preserving the no-admission-control claim boundary.

## 2026-05-11 P219 — Front-end masking evaluation package

- **Goal:** Prepare a bounded raw-vs-masked front-end masking package from P218 predicted dynamic masks, with non-SLAM proxy metrics and a concrete next-step protocol for actual ORB/DROID trajectory smoke.
- **Result: P219_FRONTEND_MASKING_EVAL_PACKAGE_READY, P195 still BLOCKED.** Added `tools/prepare_frontend_masking_eval_p219.py`, which consumes only P218 `prediction_artifacts` and P217 dataset-provided `image_path`/`binary_mask_path` rows. It generated six held-out raw/mask/masked-image package rows: `3` validation samples and `3` test samples under ignored local `outputs/frontend_masking_eval_p219/`.
- **Proxy metrics:** mean predicted masked pixel rate `0.208691`; mean GT dynamic pixel rate `0.137248`; mean mask precision/recall/F1/IoU `0.556007` / `0.789669` / `0.604636` / `0.443210`. ORB keypoint proxy was skipped because the default local OpenCV import is unavailable due to a NumPy ABI mismatch (`ImportError: numpy.core.multiarray failed to import`).
- **Scientific boundary:** P219 evaluates semantic dynamic-mask front-end inputs/proxies only. It does not run long SLAM, does not use `target_admit`, `current_weak_label`, `selection_v5`, model predictions, or human-label placeholders as admission labels, and does not claim learned admission control.
- **Outputs:** `tools/prepare_frontend_masking_eval_p219.py`, `paper/evidence/frontend_masking_eval_p219.json`, `paper/evidence/frontend_masking_eval_p219.csv`, `paper/export/frontend_masking_eval_p219.md`, plus ignored local images/manifests under `outputs/frontend_masking_eval_p219/`.
- **Verification:** `python3 tools/prepare_frontend_masking_eval_p219.py`; `python3 -m py_compile tools/prepare_frontend_masking_eval_p219.py`; `python3 tools/prepare_independent_supervision_p195.py` remains `BLOCKED` with `32/32` blank and `0/32` valid `human_admit_label`, plus `160/160` blank and `0/160` valid `human_same_object_label`.
- **P220 recommendation:** Run a tiny raw-vs-P218-masked trajectory smoke only if an existing local ORB-SLAM3 or DROID-SLAM helper is ready without downloads. Use identical backend settings, aligned timestamps and ground truth, and report trajectory availability plus ATE/RPE only when valid; keep P195 blocked and avoid admission-control claims.

## 2026-05-10 P200 — P199 semantic-stability no-label stress audit

- **Goal:** Audit whether the P199 no-human semantic-stability scorer has any geometry/session/mask-area signal beyond category-derived artifacts, without creating human labels or training admission admit/reject targets.
- **Result: AUDIT COMPLETED, skeptical boundary retained, P195 still BLOCKED.** Added a deterministic NumPy audit script for P199 stress tests. Learned P200 stress tests use only numeric geometry/area/support/session-adjacent fields and `semantic_gate_score`; `canonical_label` and semantic categories are used only for explicit degeneracy/oracle audits.
- **Category leakage audit:** non-ambiguous P199 target is deterministic by canonical category: forklift `36/36` dynamic/non-static-like, warehouse rack `16/16` static-like, work table `23/23` static-like. Leave-category-out is degenerate because every held-out category is single-class; train-on-one-category is infeasible for supervised binary learning for the same reason.
- **Feature ablations on original P199 split:** `all_numeric`, `geometry_only`, and `no_semantic_gate_score` reach test accuracy/F1(static-like) `0.7857/0.8235` but only val `0.1667/0.2857`; `area_only` and `support_only` match majority-like behavior at test `0.5238/0.6875` and val `0.3333/0.5000`. This does not establish category-independent learned semantic stability.
- **Source stress tests:** leave-source-out all-numeric logistic results are cross_day `0.8182/0.8462`, hallway `0.8095/0.8333`, cross_month `0.5000/0.4000`, same_day `0.4000/0.4000`; small source subsets and category-derived targets keep the result low-power and non-admission.
- **Baselines:** P199 test majority `0.5238/0.6875`, geometry threshold `0.4762/0.6071`, category-definition oracle `1.0000/1.0000` by construction, P199 reported MLP/logistic `0.8333/0.8571`.
- **Outputs:** `tools/audit_semantic_stability_p200.py`, `paper/evidence/semantic_stability_audit_p200.json`, `paper/export/semantic_stability_audit_p200.md`.
- **Verification:** `python3 tools/audit_semantic_stability_p200.py`; `python3 -m py_compile tools/audit_semantic_stability_p200.py tools/prepare_independent_supervision_p195.py`; `python3 tools/prepare_independent_supervision_p195.py` remains `BLOCKED` with `0/32` valid `human_admit_label` and `0/160` valid `human_same_object_label`; P194/P197 human label columns remain blank.
- **P201 recommendation:** collect/import independent human admission and same-object labels and rerun P195. If remaining no-label, treat semantic stability as an ontology/category prior unless new non-ambiguous categories cover multiple geometry and source regimes.

## 2026-05-10 P201 — No-label semantic category inventory

- **Goal:** Continue the no-human-label route by inventorying local raw TorWIC semantic segmentation assets and existing outputs for additional non-ambiguous semantic categories/samples, without converting categories into admission labels.
- **Result: INVENTORY COMPLETE, expansion BLOCKED by missing safe joins, P195 still BLOCKED.** Added a reproducible inventory script over local TorWIC extracted segmentation directories, sequence zips, `AnnotatedSemanticSet_Finetuning.zip` raw `raw_labels_2d.json` files, and existing frontend/observation manifests.
- **Raw ontology inventory:** 237 raw label JSON files; 15 semantic categories with positive pixels; 10 raw map-object/movable-clutter candidate categories. Additional raw candidates beyond P199 are `cart_pallet_jack`, `goods_material`, `misc_dynamic_feature`, `misc_non_static_feature`, and `person`. Context-only categories are kept out of object-admission rows: `wall_fence_pillar`, `ceiling`, `driveable_ground`, `ego_vehicle`, `text_region`, and `unlabeled`.
- **Existing output inventory:** 94 all-instance manifests, 93 observation indexes, 88 extracted segmentation directories, and 144,620 extracted segmentation PNGs. Existing object observations remain prompt-limited to the old object labels, safely mapping only to the P199 categories `fork_truck`, `rack_shelf`, and `fixed_machinery`; barrier/yellow-barrier remains ambiguous through `misc_static_feature;pylon_cone`.
- **Feasibility answer:** current local data cannot break the P199/P200 category=target degeneracy without guessing. The exact missing join is raw segmentation category masks/images -> `observation_id`/`tracklet_id`/`physical_key`/cross-session object cluster. Therefore no expanded semantic-stability training dataset was materialized.
- **Target policy if P202 builds the join:** category-derived `static_like` versus `dynamic_or_non_static_like` auxiliary semantic target only; context-only categories excluded from object rows; no admit/reject labels and no P193/P197 weak admission/model fields as targets.
- **Outputs:** `tools/inventory_semantic_categories_p201.py`, `paper/evidence/semantic_category_inventory_p201.json`, `paper/evidence/semantic_category_inventory_p201.csv`, `paper/export/semantic_category_inventory_p201.md`.
- **Verification:** `python3 tools/inventory_semantic_categories_p201.py`; `python3 -m py_compile tools/inventory_semantic_categories_p201.py tools/prepare_independent_supervision_p195.py`; `python3 tools/prepare_independent_supervision_p195.py` remains `BLOCKED` with `0/32` valid `human_admit_label` and `0/160` valid `human_same_object_label`; P194/P197 human label columns remain blank.
- **P202 recommendation:** build a safe raw-segmentation-to-observation join first, then rerun no-label auxiliary dataset construction. Keep P195 blocked until independent human admission/same-object labels exist.

## 2026-05-10 P202 — Raw segmentation join audit/prototype

- **Goal:** Build a safe raw-segmentation-to-observation join audit over local P193/P197/P199 rows, output observation indexes/manifests, and local TorWIC raw segmentation/RGB-D assets, without creating labels or training models.
- **Result: JOIN AUDIT COMPLETE, P195 still BLOCKED.** Added a reproducible audit script that resolves each attempted observation row through observation indexes/manifests and TorWIC date/route/frame evidence, recording exact raw segmentation color/greyscale paths and matching raw RGB/depth paths when present.
- **Join metrics:** 572 observation attempts total: P193 `110/110` exact, P197 boundary `32/32` exact, P197 pair sides `320/320` exact, and P199-derived rows `110/110` exact. Overall exact/partial/unmatched = `572/0/0`; raw evidence path existence = `3432/3432`; observation index, observation object, manifest, and manifest instance evidence each exists for `572/572` attempts.
- **AnnotatedSemanticSet blocker:** Additional categories beyond P199 remain unmatched for object-level expansion. Their numeric frame IDs can match many TorWIC routes (for example 20 route hits for sampled frames) and the zip lacks route/session/object identity, so linking `cart_pallet_jack`, `goods_material`, `misc_dynamic_feature`, `misc_non_static_feature`, `person`, and other context categories to `observation_id`/`tracklet_id`/`physical_key` would require guessing.
- **Scientific boundary:** P202 proves frame-level raw evidence joins for existing observation rows only. It does not create semantic stability/admission labels, does not use `target_admit`, `current_weak_label`, `selection_v5`, or model predictions as target labels, and does not unblock P195.
- **Outputs:** `tools/audit_raw_segmentation_join_p202.py`, `paper/evidence/raw_segmentation_join_audit_p202.json`, `paper/evidence/raw_segmentation_join_audit_p202.csv`, `paper/export/raw_segmentation_join_audit_p202.md`.
- **Verification:** `python3 tools/audit_raw_segmentation_join_p202.py`; `python3 -m py_compile tools/audit_raw_segmentation_join_p202.py tools/prepare_independent_supervision_p195.py`; `python3 tools/prepare_independent_supervision_p195.py` remains `BLOCKED` with `0/32` valid `human_admit_label` and `0/160` valid `human_same_object_label`; P194/P197 human label columns remain blank.
- **P203 recommendation:** Convert the frame-level join into a reusable no-label evidence helper, but do not materialize expanded auxiliary training rows until raw category masks can be linked to observation/object identity without ambiguous AnnotatedSemanticSet route guesses. Keep P195 blocked until independent human labels exist.

## 2026-05-10 P203 — Reusable no-label raw evidence helper

- **Goal:** Convert the P202 exact raw-frame evidence join into a reusable helper/index and CLI for downstream audits and sample review, without creating labels, training models, or expanding AnnotatedSemanticSet rows into object-level training data.
- **Result: EVIDENCE HELPER BUILT, P195 still BLOCKED.** Added `tools/raw_segmentation_evidence_helper_p203.py` with `build`, `lookup`, `summarize`, and `validate` modes. The helper reads local P202 exact rows and writes a compact evidence index with row provenance, sample/observation identity, session/frame, raw segmentation color/greyscale, RGB/depth paths, canonical label, tracklet id, and physical key only.
- **Index counts:** `572` exact evidence rows: P193 `110`, P197 boundary `32`, P197 pair sides `320`, P199 `110`. Category counts are forklift `196`, barrier `157`, work table `129`, warehouse rack `90`. Raw evidence path validation is `3432/3432` existing.
- **No-label boundary:** The P203 index excludes human labels, weak labels, `target_admit`, model probabilities/predictions, and selection fields. It does not materialize auxiliary training rows from richer AnnotatedSemanticSet categories and does not claim admission control.
- **Outputs:** `tools/raw_segmentation_evidence_helper_p203.py`, `paper/evidence/raw_segmentation_evidence_index_p203.json`, `paper/evidence/raw_segmentation_evidence_index_p203.csv`, `paper/export/raw_segmentation_evidence_helper_p203.md`.
- **Verification:** `python3 tools/raw_segmentation_evidence_helper_p203.py build`; lookup smoke by one P193 `sample_id` returned 4 evidence rows and lookup by one P197 pair-side `observation_id` returned 11 evidence rows; `python3 tools/raw_segmentation_evidence_helper_p203.py summarize`; `python3 tools/raw_segmentation_evidence_helper_p203.py validate` reports all paths exist; `python3 -m py_compile tools/raw_segmentation_evidence_helper_p203.py tools/prepare_independent_supervision_p195.py`; `python3 tools/prepare_independent_supervision_p195.py` remains `BLOCKED` with `0/32` valid `human_admit_label` and `0/160` valid `human_same_object_label`; P194/P197 human label columns remain blank.
- **P204 recommendation:** Use the helper for reviewer/audit sampling and raw evidence packet inspection. Do not expand AnnotatedSemanticSet categories into object-level training rows until route/session/object identity is available for a safe join; keep P195 blocked until independent human labels exist.

## 2026-05-10 P204 — No-label raw evidence audit packet

- **Goal:** Use the P203 helper/index to build a representative raw evidence packet for sample inspection across category/source/session/row-source, without creating labels or training models.
- **Result: AUDIT PACKET BUILT, P195 still BLOCKED.** Added a deterministic packet builder that samples from `paper/evidence/raw_segmentation_evidence_index_p203.csv` using coverage over canonical category, source, row source, observed pairings, and session diversity. The packet is inspection-only and includes raw RGB/depth/segmentation paths plus blank non-label note fields.
- **Packet coverage:** `32` samples; categories barrier `17`, forklift `6`, warehouse rack `4`, work table `5`; sources cross_day `17`, cross_month `4`, hallway `7`, same_day `4`; row sources P193 `13`, P197 boundary `6`, P197 pair `8`, P199 `5`; `12` sessions represented. All target categories, all target sources, and all target row-source types are covered.
- **Evidence validation:** raw evidence paths are `192/192` existing. Non-label inspection note fields are blank. Packet columns exclude human labels, weak targets, `target_admit`, selection fields, and model predictions/probabilities.
- **Outputs:** `tools/build_raw_evidence_audit_packet_p204.py`, `paper/evidence/raw_evidence_audit_packet_p204.json`, `paper/evidence/raw_evidence_audit_packet_p204.csv`, `paper/export/raw_evidence_audit_packet_p204.md`.
...
## 2026-05-10 P206 — Diversity-first no-label raw evidence packet

- **Goal:** Rebuild the no-label raw evidence audit packet with diversity as the first priority, minimizing duplicate `sample_id`, duplicate `session_id`/`frame_index`, and duplicate nonblank `physical_key`, while preserving useful category/source/row-source/session coverage.
- **Result: DIVERSE PACKET BUILT, P195 still BLOCKED.** Added `tools/build_diverse_raw_evidence_packet_p206.py`, which deterministically selects from the P203 index under strict unique-sample, unique-session/frame, and unique-nonblank-physical-key constraints, then runs a P206-specific QA triage using the P205 image/readability checks.
- **Packet coverage:** `32` samples; categories are balanced at barrier `8`, forklift `8`, warehouse rack `8`, work table `8`; sources cross_day `8`, cross_month `4`, hallway `16`, same_day `4`; row sources P193 `6`, P197 boundary `6`, P197 pair `14`, P199 `6`; `18` sessions represented.
- **Diversity result:** duplicate sample-id groups `0` (P205/P204: `7` groups affecting `15` rows), duplicate session/frame groups `0` (P205/P204: `9` groups affecting `23` rows), duplicate nonblank physical-key groups `0`. P197 boundary/pair rows still have blank `physical_key` because the P203 index does not carry physical keys for those row sources.
- **Evidence QA:** `192/192` referenced paths exist and are readable as images; all 32 rows have internally consistent `1280x720` dimensions; all 4 segmentation images per row are non-trivial; depth images are readable with nonzero pixels. There are `0` error-level issues and `0` warning-level issues.
- **Tradeoffs:** Strict diversity left three secondary pairing tokens uncovered: `category_row_source:forklift|p197_boundary`, `source_row_source:cross_month|p197_boundary`, and `source_row_source:same_day|p193`. All required categories, sources, row sources, and broad session coverage are still present.
- **Outputs:** `tools/build_diverse_raw_evidence_packet_p206.py`, `paper/evidence/raw_evidence_diverse_packet_p206.json`, `paper/evidence/raw_evidence_diverse_packet_p206.csv`, `paper/export/raw_evidence_diverse_packet_p206.md`, `paper/evidence/raw_evidence_diverse_packet_triage_p206.json`, `paper/evidence/raw_evidence_diverse_packet_triage_p206.csv`, `paper/export/raw_evidence_diverse_packet_triage_p206.md`.
- **Verification:** `python3 tools/build_diverse_raw_evidence_packet_p206.py`; `python3 -m py_compile tools/build_diverse_raw_evidence_packet_p206.py tools/triage_raw_evidence_packet_p205.py tools/prepare_independent_supervision_p195.py`; JSON sanity confirms 32 rows, no prohibited label/proxy columns, blank non-label note fields, and zero missing referenced paths; `python3 tools/prepare_independent_supervision_p195.py` remains `BLOCKED` with `0/32` valid `human_admit_label` and `0/160` valid `human_same_object_label`.
- **P207 recommendation:** Use the P206 packet as the preferred no-label raw-evidence audit set. Keep P195 blocked until independent human admission and same-object labels exist; if continuing no-label work, focus next on a visual evidence-quality review protocol that records only raw-evidence issues, not admission/same-object labels.

## 2026-05-10 P207 — Evidence-quality notes protocol

- **Goal:** Create a no-training, no-label visual evidence quality notes protocol and safe notes template for the P206 packet, limited to visibility/occlusion/blur/mask alignment/depth usability.
- **Result: TEMPLATE AND PROTOCOL BUILT, P195 still BLOCKED.** Added `tools/build_evidence_quality_notes_p207.py` with `build` and `validate` modes. The build mode reads the P206 diverse raw evidence packet, preserves row identifiers and raw evidence paths, and writes blank evidence-quality note fields only. The validate mode checks allowed values, evidence path existence, duplicate/blank audit IDs, prohibited label/proxy columns, and possible admission/same-object decision text in reviewer notes.
- **Note schema:** `visibility_quality` (`clear`, `partial`, `poor`, `not_assessable`), `mask_alignment_quality` (`good`, `minor_issue`, `major_issue`, `not_assessable`), `depth_quality` (`usable`, `limited`, `unusable`, `not_assessable`), `occlusion_level`/`blur_level` (`none`, `low`, `medium`, `high`, `not_assessable`), `quality_blocker` (`yes`, `no`), and free-text `reviewer_note`; blank is valid for all note fields and all generated values are blank.
- **Current template QA:** 32 rows from P206, category balance remains barrier/forklift/warehouse rack/work table `8` each, raw evidence paths are `192/192` existing, and all seven note fields are blank for all rows. CSV columns contain no `human_admit_label`, `human_same_object_label`, weak target, selection, or model prediction/probability fields.
- **Protocol boundary:** P207 is evidence QA only. It explicitly forbids recording admit/reject or same/different-object decisions, copying weak labels/category names as labels, training models, editing raw data, or using P207 notes for admission-control claims.
- **Outputs:** `tools/build_evidence_quality_notes_p207.py`, `paper/evidence/raw_evidence_quality_notes_p207.csv`, `paper/evidence/raw_evidence_quality_notes_p207.json`, `paper/export/raw_evidence_quality_notes_protocol_p207.md`.
...
## 2026-05-10 P210 — Evidence-quality note ingestion audit

- **Goal:** Add read-only ingestion audit coverage for future filled P207 evidence-quality notes, proving whether notes are ingestible and still strictly quality-only.
- **Result: INGESTION AUDIT READY, P195 still BLOCKED.** Added `tools/audit_evidence_note_ingestion_p210.py`, which reads P207 notes, reuses P207/P208 validation behavior, classifies ingestion readiness, and emits a quality-only downstream manifest that excludes human/admission/same-object/proxy fields and also excludes `canonical_label`.
- **Readiness classifier:** Current real P207 notes return `READY_EMPTY` because all seven note fields remain blank. Synthetic temporary cases cover `READY_EMPTY`, `READY_WITH_QUALITY_NOTES`, `BLOCKED_INVALID_VALUES`, and `BLOCKED_LABEL_LEAKAGE` for both prohibited columns and admission/same-object-like reviewer text.
- **Manifest boundary:** The generated P210 manifest contains only row identity/provenance, local raw-evidence paths, and P207 quality-note fields. It contains no `human_admit_label`, no `human_same_object_label`, no admission/same-object decision fields, and no model/weak/selection proxy fields.
- **Scientific boundary:** P210 audits evidence-quality note ingestion only. It does not fill or infer admission/same-object labels, does not import labels, does not train, does not modify real P207 notes, and does not support admission-control claims.
- **Outputs:** `tools/audit_evidence_note_ingestion_p210.py`, `paper/evidence/evidence_note_ingestion_audit_p210.json`, `paper/export/evidence_note_ingestion_audit_p210.md`, `paper/evidence/evidence_note_ingestion_manifest_p210.csv`.
- **Verification:** `python3 tools/audit_evidence_note_ingestion_p210.py` returns `PASS` with readiness `READY_EMPTY`, 32 rows, blank note fields, manifest safety checks false, self-test `PASS`, and real P207 notes SHA-256 unchanged; `python3 tools/audit_evidence_note_ingestion_p210.py --self-test` returns `PASS` with 5/5 cases; `python3 -m py_compile tools/build_evidence_quality_notes_p207.py tools/summarize_evidence_quality_notes_p208.py tools/test_evidence_quality_safety_p209.py tools/audit_evidence_note_ingestion_p210.py tools/prepare_independent_supervision_p195.py`; `python3 tools/prepare_independent_supervision_p195.py` remains `BLOCKED` with `0/32` valid `human_admit_label` and `0/160` valid `human_same_object_label`; real P207 notes remain blank and hash unchanged.
- **P211 recommendation:** Keep P195 blocked until independent human labels exist. If staying no-label, add a read-only, cross-phase evidence packet index that links P206/P207/P208/P210 outputs and flags stale or mismatched artifact hashes without introducing labels.

## 2026-05-10 P211 — Cross-phase no-label evidence index

- **Goal:** Add a read-only cross-phase evidence index and staleness checker for the P206-P210 no-label evidence QA stack, linking packet, notes, summary, safety-test, and ingestion-audit outputs without introducing labels or training data.
- **Result: INDEX BUILT, STALENESS PASS, P195 still BLOCKED.** Added `tools/build_cross_phase_evidence_index_p211.py`, which records artifact paths, existence, size, SHA-256, obvious row counts, generated phase, role, and upstream dependencies for P206-P210 scripts/reports/artifacts.
- **Indexed artifacts:** `22` artifacts total: P206 diverse packet/triage outputs and script, P207 notes/protocol and script, P208 summary/blocker outputs and script, P209 safety-test report/script, and P210 ingestion-audit/manifest outputs and script.
- **Staleness checks:** `PASS` for required artifact existence, P206 triage zero issues, P207 row IDs matching P206 packet rows, P207 notes still blank, P208 summary counts matching current P207 notes, P209 safety-test report passing against current P207 hash, P210 audit hash/readiness/self-test consistency, P210 manifest quality-only row match, generated P211 CSV schema safety, and P195 BLOCKED propagation.
- **Boundary:** P211 is artifact governance only. It does not fill or infer `human_admit_label` or `human_same_object_label`, does not create admission/same-object labels, does not modify real notes or raw data, does not train, and does not support admission-control claims. The artifact-level CSV is not an ingestible row-level evidence table and contains no row-level decision/proxy columns.
- **Outputs:** `tools/build_cross_phase_evidence_index_p211.py`, `paper/evidence/cross_phase_evidence_index_p211.json`, `paper/evidence/cross_phase_evidence_index_p211.csv`, `paper/export/cross_phase_evidence_index_p211.md`.
- **Verification:** `python3 tools/build_cross_phase_evidence_index_p211.py` returns `PASS` with 22 artifacts and staleness `PASS`; `python3 -m py_compile tools/build_cross_phase_evidence_index_p211.py tools/prepare_independent_supervision_p195.py`; `python3 tools/test_evidence_quality_safety_p209.py` returns `PASS` with 6/6 cases and real P207 notes unchanged; `python3 tools/prepare_independent_supervision_p195.py` remains `BLOCKED` with `0/32` valid `human_admit_label` and `0/160` valid `human_same_object_label`; generated P211 CSV columns contain no prohibited row-level decision/proxy fields.
- **P212 recommendation:** Keep P195 blocked until independent human admission/same-object labels exist. If staying no-label, add a read-only release-style governance summary that compares artifact hashes across commits and flags drift before paper export.

## 2026-05-10 P212 — Release no-label evidence governance summary

- **Goal:** Add a release/export readiness governance checker for the P206-P211 no-label evidence stack using the current local repo state and P211 index, without creating labels, training, downloading data, or modifying raw evidence/real notes.
- **Result: GOVERNANCE SUMMARY BUILT, RELEASE BLOCKED BY DIRTY RELEVANT ARTIFACTS, P195 still BLOCKED.** Added `tools/build_release_governance_summary_p212.py`, which records the current git commit, P206-P211 artifact hash comparison against the P211 index, scoped dirty tracked/untracked status, P195 gate state, P209 safety state, P210 readiness, label/proxy leakage checks, ignored protected untracked paths, and an explicit no-label claim boundary.
- **Release readiness:** `BLOCKED_STALE_ARTIFACTS`. The 22 P206-P211 artifact hashes match the P211 index, P209/P210/P195 checks pass, and P212 outputs contain no prohibited row-level decision/proxy fields; however, release/export is blocked because relevant tracked files `paper/evidence/raw_evidence_diverse_packet_p206.json` and `paper/evidence/raw_evidence_diverse_packet_triage_p206.json` are dirty in the current worktree.
- **Ignored local noise:** protected untracked `thirdparty/` and `tools/orb_slam3_headless*` paths are reported separately and do not cause P212 to fail.
- **Scientific boundary:** P212 is paper/export governance for no-label evidence only. It does not fill or infer `human_admit_label` or `human_same_object_label`, does not create admission/same-object labels, does not train models, and does not support admission-control claims.
- **Outputs:** `tools/build_release_governance_summary_p212.py`, `paper/evidence/release_governance_summary_p212.json`, `paper/export/release_governance_summary_p212.md`.
...
## 2026-05-11 P215 — Evidence-only appendix/index integration

- **Goal:** Integrate the P214 evidence-only release bundle into the `paper/export` appendix/index path so the P206-P214 no-label governance evidence is easy to cite and review, without changing scientific claims or creating labels/training data.
- **Result: APPENDIX INDEX BUILT, P195 still BLOCKED.** Added `tools/build_evidence_appendix_index_p215.py`, which reads `paper/evidence/evidence_only_release_bundle_p214.json` and deterministically writes `paper/export/evidence_only_appendix_index_p215.md`.
- **Integration point:** `paper/export/evidence_only_appendix_index_p215.md` sits beside the existing P214, P212, and P211 export reports. It links the P214 Markdown/JSON/CSV bundle and summarizes the P206-P214 artifact trail with status, path, SHA-256, and claim boundary.
- **Claim boundary:** Evidence-only governance is ready; P195 learned admission control remains `BLOCKED`; no independent human labels exist; no admission/same-object labels were generated; semantic/category evidence is reviewer/audit evidence only and is not admission or same-object ground truth.
- **Verification:** `python3 tools/build_evidence_appendix_index_p215.py`; `python3 -m py_compile tools/build_evidence_appendix_index_p215.py`; `python3 tools/prepare_independent_supervision_p195.py` remains `BLOCKED` with `32/32` blank and `0/32` valid `human_admit_label`, plus `160/160` blank and `0/160` valid `human_same_object_label`; `paper/evidence/evidence_only_release_bundle_p214.json` remains `READY_NO_LABEL_EVIDENCE_ONLY`; P214 CSV and P215 index table headers contain no prohibited row-level decision/proxy columns.
- **P216 recommendation:** Keep the next step on the no-label governance boundary unless independent human admission and same-object labels are collected. Recommended P216 options are either final paper-reference cleanup around the P215 appendix index, or a separate label-collection/import gate that preserves semantic/category evidence as audit evidence only.
...
- **Diff finding:** The two release-blocking P206 JSONs were benign generated timestamp drift only (`generated_at_utc`). Other tracked dirty P197-P205 evidence JSONs were also one-line timestamp-only drift and were left uncommitted because they are outside the P206-P212 release-governance cleanup scope.
- **Result: RELEASE READY FOR NO-LABEL EVIDENCE ONLY, P195 still BLOCKED.** Rebuilt the P206 diverse packet through the existing builder, accepted the reproducible generated P206 timestamp refresh, refreshed P211 hashes/mtimes for the changed P206 JSON artifacts, fixed P212 porcelain parsing so dirty-scope reporting preserves leading status spaces, and rebuilt P212 against the cleaned P206-P211 artifact state.
- **Release readiness:** `READY_NO_LABEL_EVIDENCE_ONLY`. P211 artifact hash comparison is `PASS` for 22 artifacts, relevant P206-P211 dirty tracked/untracked status is clean, P209 safety remains `PASS` with `6/6` cases, P210 remains `READY_EMPTY`, P195 remains `BLOCKED`, and label/proxy leakage scans remain `PASS`.
- **Label boundary:** `human_admit_label` remains blank/valid/total `32/0/32`; `human_same_object_label` remains blank/valid/total `160/0/160`. No admission/same-object labels were filled, inferred, copied, or created; no training was run.
- **Verification:** `python3 tools/build_diverse_raw_evidence_packet_p206.py` returns 32 rows, zero P206 triage issues, and 192/192 raw evidence paths existing/readable; `python3 tools/build_cross_phase_evidence_index_p211.py` returns `PASS` with 22 artifacts and staleness `PASS`; `python3 tools/build_release_governance_summary_p212.py` returns `READY_NO_LABEL_EVIDENCE_ONLY`; `python3 tools/test_evidence_quality_safety_p209.py` returns `PASS` with `6/6` cases and unchanged real P207 notes hash; `python3 tools/prepare_independent_supervision_p195.py` remains `BLOCKED`; `python3 -m py_compile tools/build_diverse_raw_evidence_packet_p206.py tools/build_cross_phase_evidence_index_p211.py tools/build_release_governance_summary_p212.py tools/test_evidence_quality_safety_p209.py tools/prepare_independent_supervision_p195.py` passes; generated P206/P211/P212/P207/P210 CSV/schema scans show no prohibited label/proxy columns.
- **P214 recommendation:** Keep the release boundary at no-label evidence governance. Either collect/import independent P195/P196 human labels through the existing protocol, or package the current P206-P212 evidence stack as evidence-only support without admission-control training claims.
