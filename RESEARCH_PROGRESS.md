...
- **ORB-SLAM3 cross-check (P173.1):** ✅ Metrics computed — evo unblocked (scipy 1.15.3 user-level upgrade), APE/RPE computed on sparse keyframe trajectories against groundtruth. Raw (10 KFs): APE RMSE 0.052 m, RPE RMSE 0.024 m. Masked (12 KFs): APE RMSE 0.048 m, RPE RMSE 0.020 m. ORB-SLAM3 tracked only ~1.2s of the 7s sequence (17%). Metrics mathematically valid but low statistical power (N=10–12). No improvement claim: these numbers merely document that ORB-SLAM3 briefly initialized and tracked, and that masking was marginally better inside that short window. Report: `paper/export/orb_slam3_cross_check_p173_metrics.md`. JSON evidence: `paper/evidence/orb_slam3_cross_check_p173_metrics.json`.
