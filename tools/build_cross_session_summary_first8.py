"""
Build a cross-session summary over the first-8-frame industrial runs.

This extends the earlier first-frame-only summary by aggregating all available map_objects.json
files from the first eight frames of each TorWIC revisit session.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--root', type=Path, default=Path('/home/rui/slam/outputs'))
    p.add_argument('--output', type=Path, required=True)
    p.add_argument('--max-center-distance', type=float, default=220.0)
    p.add_argument('--max-size-ratio-diff', type=float, default=0.55)
    return p.parse_args()
...
def discover_inputs(root: Path) -> list[Path]:
    paths = []
    for pattern in [
        'torwic_jun15_aisle_cw_run1_f*/map_output/map_objects.json',
        'torwic_jun23_aisle_cw_run1_f*/map_output/map_objects.json',
        'torwic_oct12_aisle_cw_f*/map_output/map_objects.json',
    ]:
...
def main() -> None:
    ...
    for path in inputs:
        objs = load_json(path)
        frame_name = path.parent.parent.name
        if frame_name.startswith('torwic_jun15'):
            source_session = 'Jun15'
        elif frame_name.startswith('torwic_jun23'):
            source_session = 'Jun23'
        elif frame_name.startswith('torwic_oct12'):
            source_session = 'Oct12'
        else:
            source_session = frame_name
        for obj in objs:
            x = dict(obj)
            x['source_session'] = source_session
            x['source_frame'] = frame_name
            items.append(x)
    ...
    print(json.dumps({'output': str(args.output), 'num_inputs': len(inputs), 'num_clusters': len(clusters)}, indent=2, ensure_ascii=False))
