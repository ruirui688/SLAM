...
BUILD_CUSTOM_OPS = os.getenv("GROUNDINGDINO_BUILD_OPS", "1") == "1"

torch_ver = [int(x) for x in torch.__version__.split(".")[:2]]


def get_extensions():
    if not BUILD_CUSTOM_OPS:
        print("Skipping GroundingDINO custom ops build because GROUNDINGDINO_BUILD_OPS=0.")
        return None

    if os.name == "nt" and shutil.which("cl") is None:
        print("MSVC cl.exe not found on PATH. Skipping GroundingDINO custom ops build.")
        return None
