...
This paper identifies and addresses a gap in current semantic SLAM architectures: the transition from open-vocabulary object detection output to durable semantic map entries requires an explicit admission-control policy that current pipelines lack. We present a session-level map maintenance architecture with four layers—observation, tracklet, map-object, and revision—that scores each candidate object on stability, persistence, and dynamicity before admitting it to the semantic map. Evaluated on the TorWIC industrial dataset across 35 sessions spanning same-day to cross-month revisits, the framework consistently retains infrastructure (barriers, work tables, racks) while rejecting 100% of dynamic contamination (forklifts). A systematic parameter ablation reveals natural data bimodality that makes the admission policy robust to threshold selection. A 3-way baseline comparison demonstrates that simpler heuristics (purity/support proxy) fail to reject dynamic objects (phantom risk 21.1% vs. 0.0% with the full policy). A complete 10-configuration negative-result study quantifies the boundary condition under which dynamic pixel masking does not affect visual odometry in the TorWIC environment. The contribution is bounded: not a lifelong SLAM benchmark, not dense dynamic reconstruction, and not a downstream navigation-gain claim. The supplementary package provides per-cluster evidence traceable to individual observation IDs so that all admission decisions are auditable.
...
| Negative result | 10-config dynamic SLAM boundary condition | ✅ Honest limitations |
