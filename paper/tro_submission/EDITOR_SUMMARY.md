...
**Statement:** Dynamic pixel masking in the TorWIC warehouse environment does not improve visual odometry accuracy, but mask selectivity matters. Across 14 DROID-SLAM configurations on 3 TorWIC Aisle sessions (P171 single-session + P172 Stage~1 cross-session replication), two groups emerge: 9/14 trajectory-neutral ($|\Delta\text{APE}| \leq 0.006$~mm) with selective masks (semantic frontend, concentrated forklift); 5/14 perturbed ($0.92$--$7.52$~mm) with aggressive masks (temporal/flow propagation, uniform first-N). Stage~1 replication confirms cross-session reproducibility on Jun~15 Run~2 (same-day) and Jun~23 Run~1 (cross-day). Both sessions produce $\Delta\text{APE} \leq 0.001~\text{mm}$ with 4/64 frames masked and max coverage $\leq 1.49\%$.
...
Only DROID-SLAM was tested (14 configurations, 3 TorWIC Aisle sessions). ORB-SLAM3, Hallway scene transfer, and cross-month temporal separation ($>$~2 weeks) are not yet evaluated. Listed as future work.
