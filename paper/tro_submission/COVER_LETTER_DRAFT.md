...
**Key evidence:** The framework is evaluated on the TorWIC industrial dataset (POV-SLAM, RSS 2023) across three Aisle revisits (same-day, cross-day, cross-month) and a Hallway scene-transfer protocol. Supporting evidence includes: (i) a 27-combination parameter ablation confirming natural data bimodality; (ii) a 3-baseline comparison (naive-all-admit, purity/support heuristic, and our cross-session policy) demonstrating that simpler heuristics cannot reject dynamic contamination; (iii) per-category retention and rejection analysis with a 5-reason taxonomy; and (iv) a complete 10-configuration negative-result study showing that dynamic pixel masking is trajectory-neutral in the TorWIC environment (|ΔATE| < 0.1 mm), with a quantified boundary condition.
