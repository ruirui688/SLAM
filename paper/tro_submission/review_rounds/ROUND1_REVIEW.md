...
**Rebuttal direction:**
- We acknowledge the small-N limitation. The contribution is architectural, not statistical. The 20 clusters represent all object-level entities detected in the current physical-session evaluation bundle (20 TorWIC sessions, with 35 map-object artifact files/variants in the maintained evidence bundle) — this is the within-site population for the reported protocol, not a random sample. The replication question is about transfer to other sites, not sampling error within this site.
