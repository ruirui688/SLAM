# P194 No-Proxy Admission-Scorer CUDA Stress
...
**Actual command:** `LD_LIBRARY_PATH=/home/rui/miniconda3/envs/tram/lib: conda run -n tram python tools/train_admission_scorer_gpu_p192.py --dataset paper/evidence/admission_frame_dataset_p193.csv --dataset-kind frame-observation-no-proxy-stress --drop-features dynamic_ratio,label_purity,is_forklift_like,is_infrastructure_like --p191-json paper/evidence/admission_scorer_smoke_p191.json --p192-json paper/evidence/admission_scorer_frame_gpu_p193.json --comparison-label 'P193 full-proxy frame GPU baseline' --output-json paper/evidence/admission_scorer_no_proxy_p194.json --output-md paper/export/admission_scorer_no_proxy_p194.md --phase P194-no-proxy-leakage-stress --report-title 'P194 No-Proxy Admission-Scorer CUDA Stress'`
...
**Dataset:** `paper/evidence/admission_frame_dataset_p193.csv` (110 samples; dataset kind `frame-observation-no-proxy-stress`).
...
- memory_after: allocated=17044992 bytes, reserved=23068672 bytes
- memory_peak: allocated=18107392 bytes, reserved=23068672 bytes

## Metrics: GPU MLP vs GPU Logistic vs Rule Baseline

### train
- GPU MLP: accuracy=0.9762, precision=0.9655, recall=1.0000, F1=0.9825, fp=1, fn=0.
- GPU logistic: accuracy=0.9048, precision=0.9000, recall=0.9643, F1=0.9310, fp=3, fn=1.
- Rule baseline: accuracy=0.3333, precision=0.0000, recall=0.0000, F1=0.0000, fp=0, fn=28.

### val
- GPU MLP: accuracy=0.5000, precision=0.5000, recall=0.7500, F1=0.6000, fp=3, fn=1.
- GPU logistic: accuracy=0.7500, precision=1.0000, recall=0.5000, F1=0.6667, fp=0, fn=2.
- Rule baseline: accuracy=0.5000, precision=0.0000, recall=0.0000, F1=0.0000, fp=0, fn=4.

### test
- GPU MLP: accuracy=0.8667, precision=0.8372, recall=0.9730, F1=0.9000, fp=7, fn=1.
- GPU logistic: accuracy=0.7833, precision=0.8000, recall=0.8649, F1=0.8312, fp=8, fn=5.
- Rule baseline: accuracy=0.3833, precision=0.0000, recall=0.0000, F1=0.0000, fp=0, fn=37.

## P191 CPU Baseline Comparison

- train: P191 CPU all-features acc/F1=1.0000/1.0000; current GPU MLP acc/F1=0.9762/0.9825.
- val: P191 CPU all-features acc/F1=0.8571/0.8333; current GPU MLP acc/F1=0.5000/0.6000.
- test: P191 CPU all-features acc/F1=0.6875/0.7368; current GPU MLP acc/F1=0.8667/0.9000.

## P193 full-proxy frame GPU baseline Comparison

- train: P193 full-proxy frame GPU baseline MLP acc/F1=1.0000/1.0000; current GPU MLP acc/F1=0.9762/0.9825.
- val: P193 full-proxy frame GPU baseline MLP acc/F1=1.0000/1.0000; current GPU MLP acc/F1=0.5000/0.6000.
- test: P193 full-proxy frame GPU baseline MLP acc/F1=0.9500/0.9610; current GPU MLP acc/F1=0.8667/0.9000.

## Training Runtime

- GPU MLP training time: 0.281001 s
- GPU logistic training time: 0.232927 s

## Risk / Interpretation

- This really used CUDA, but remains a bounded experiment: dataset kind `frame-observation-no-proxy-stress` and labels are still weak labels unless independently reviewed.
