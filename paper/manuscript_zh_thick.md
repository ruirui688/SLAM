...
在仓库、物流中心和制造车间等工业环境中，长期视觉SLAM面临一个根本性挑战：开放词汇检测管线天然地会在每一帧中产生数十个候选目标实例，但其中只有一小部分对应着可持久化、可复用的地图实体[1]。叉车、推车、托盘和临时物体如果被直接插入地图会污染地图——这是动态SLAM文献[2]中认识到的动态污染问题，但尚未在地图准入层面得到处理；而护栏、工作台、仓储货架虽然稳定，但在视角或光照变化下可能只是间歇性出现。本文提出一种面向对象的语义分割辅助SLAM方法，在感知与地图之间引入显式的对象维护层。该框架将开放词汇RGB-D分割输出转化为观察层（Observation）、轨迹层（Tracklet）、地图对象层（MapObject）和修订层（Revision），然后利用持久性、稳定性和动态性评分来保留稳定的语义地标并抑制动态污染。我们在TorWIC数据集（POV-SLAM来源[6]）上评估该框架，在当日（203个观测/11个候选簇→5个保留）、跨日（240/10→5）和跨月（297/14→7）三个更丰富的过道（Aisle）协议上（§VII.A），报告了一个可复现的投稿级证据阶梯。独立的走廊（Hallway）广泛验证分支（537个观测/16个候选簇→9个保留，在80/80执行帧上）提供了场景迁移证据（§VII.C）。该框架在全部四个协议上以一致的50.0%–71.4%拒绝比例将叉车类证据作为动态污染拒绝（§VII.D），同时保留满足持久性和一致性标准（§V.E）的稳定非移动物体。我们将该贡献定位为一个有边界的IEEE系统类贡献：不是最终的终身SLAM基准[1]，不是稠密动态重建，也不是下游导航增益声明，而是一座从分割辅助的开放词汇感知[3][4][5]到实际工业回访中长期对象地图维护[6]的可审计桥梁。
...
1. **显式的对象维护架构**（§V）：包含观察（§V.A）、轨迹（§V.B）、地图对象（§V.C）和修订（§V.F）四个层次，将检测与地图准入分离。每一层都携带帧级来源、稳定性计数器和动态性评分。

2. **透明的地图准入住信度评分**（§V.D）：结合会话级稳定性、跨会话持久性和主导状态动态性。该评分故意保持简单和可审计，而非作为最优估计器呈现。

3. **可复现的证据阶梯**（§VII.A–VII.C）：在TorWIC数据集[6]上，通过三个过道协议（当日、跨日、跨月）和一个走廊广泛验证分支，全部以选择标准论证（§VI.C）和簇级可追溯性（附录Y）报告。

4. **拒绝画像分析**（§VII.D）：叉车类动态污染是跨所有协议的主要拒绝驱动因素（50.0%–71.4%占比），会话覆盖不足和标签纯度为第二/第三驱动因素。

5. **开源发布**：对象维护层连同TorWIC协议配置（§VI）和选择标准（§V.E）一起开源，支持对所报告证据阶梯的可审计复现。
...
1. **叉车被以高置信度检测到。** 检测器工作正常——它识别了叉车。在我们的过道协议（§VII.A）中，叉车类簇的平均检测置信度与保留的基础设施物体相当，但全部16个被准入标准（§V.E, §VII.D）正确拒绝为动态污染。高检测置信度不区分"这是一个真实的叉车"和"这是一个值得进入地图的静态地标"。
...
当前阈值（min_sessions=2）代表一个保守选择。在部署中，更长的观察窗口将允许单会话稳定物体在准入前积累证据——这一权衡在跨月协议（§VII.A）中可见，该协议中两个额外物体在10月会话后被准入。
...
当前的P108-P119 + P125-P156审计链（见closure bundle v36）对全部十条正式引用[1]–[10]及evo软件引用[S]进行了元数据验证，并包含字段级DOI完整性检查。附录级消融实验（P154）和基线对比（P155）提供了额外证据，表明准入标准不是超参数调优产物，且简单启发式策略无法达到与完整跨session策略相同的区分力（§附录）。最终成果支撑一个有边界的IEEE系统类贡献：不是最终的终身SLAM基准，不是稠密动态重建，也不是下游导航增益声明，而是一座从分割辅助的开放词汇感知[3][4][5][7][8][9]到实际工业回访中修订式长期对象地图维护[6]的可审计桥梁。
...
**当前检索入口：** `/home/rui/slam/outputs/torwic_submission_ready_closure_bundle_v36.md`（P108-P156自主打磨完成）。

---

## 附录：准入标准消融实验（P154）

对组合数据集（Aisle + Hallway，35个 map_objects.json 文件，762个原始地图对象，20个跨 session 聚类）进行了准入标准敏感性分析，扫描 min_sessions (1/2/3)、min_frames (2/4/6)、max_dynamic_ratio (0.10/0.20/0.30)。min_support=6 和 min_label_purity=0.70 保持默认。

**参数扫描结果：**

| 参数 | 取值 | 选中 | 拒绝 | 敏感性 |
|---|---|---|---|---|
| min_sessions | 1 → 2 → 3 | 7 → 5 → 5 | 13 → 15 → 15 | 敏感 (1→2) |
| min_frames | 2 → 4 → 6 | 8 → 5 → 5 | 12 → 15 → 15 | 敏感 (2→4) |
| max_dynamic_ratio | 0.10 → 0.20 → 0.30 | 5 → 5 → 5 | 15 → 15 → 15 | 不敏感 |

**默认结果（5 选中，15 拒绝）：** 入选的全部为基础设施（yellow barrier/work table），dynamic_ratio=0.00。拒绝原因：7 单 session、4 帧数不足、3 支撑度不足、1 动态污染。

**关键发现：**

1. **min_sessions=2 是影响最大的过滤器。** 降低至 1 会额外引入 2 个单 session 噪声聚类。提高至 3 无影响。
2. **min_frames=4 提供空间多样性约束。** 降低至 2 会额外引入 3 个低帧数聚类。
3. **max_dynamic_ratio 自然饱和。** 数据呈现双峰分布：基础设施 ratio=0.00，叉车 ratio≥0.83。任何 [0.01, 0.82] 内的阈值产生相同结果。0.20 是保守默认值而非调参产物。

这些标准不是为特定指标调优的任意超参数。min_sessions 和 min_frames 构成实际过滤器并表现出可见敏感性，而 max_dynamic_ratio 利用了数据中基础设施（静止）和移动体（运动）之间的固有分离。消融实验确认默认参数下的 5 个稳定对象对参数扰动是稳定的，而 15 个被拒绝的聚类在任何合理的阈值组合下都不会被准入。

**完整扫描输出：** `outputs/torwic_admission_ablation_results.json`
...
![Fig. 12. 对象生命周期：稳定障碍物准入 vs 叉车拒绝。](figures/torwic_object_lifecycle_p156.png)

![Fig. 13. 准入决策空间：全部 20 个聚类的 dynamic_ratio vs session_count。](figures/torwic_admission_decision_space_p156.png)

在组合数据集（Aisle + Hallway，20 个跨 session 聚类）上对比了三种地图准入策略，量化更丰富准入策略相比简单基线的增益。

*图 11 展示地图组成变化：B0 准入全部 20 个聚类含 4 个叉车（20% 幻影风险）；B2 仅保留 5 个可验证基础设施（0% 幻影）。图 12 展示一个稳定障碍物（准入——满足全部 5 项标准）与一个叉车（拒绝——dynamic_contamination，尽管纯度和支撑度较高）的生命周期。图 13 映射全部 20 聚类的决策空间：基础设施聚类（蓝/黄/紫）集中在 dynamic_ratio=0.00 且 sessions≥2；叉车聚类（红）位于 dynamic_ratio≥0.83，与基础设施在 dynamic_ratio 轴上自然分离 ≥0.83。*
...
| ID | 策略 | 描述 |
|---|---|---|
| B0 | 全量准入（Naive） | 不做任何过滤，所有聚类准入地图 |
| B1 | 纯度/支撑度代理（Purity/Support） | 若 label_purity ≥ 0.85 或 support ≥ 10 则准入（替代缺失的逐检测置信度；限制见下文） |
| B2 | 丰富准入（Richer，当前默认） | 完整五标准策略：min_sessions≥2, min_frames≥4, min_support≥6, max_dynamic_ratio≤0.20, min_label_purity≥0.70 |

**结果：**

| 指标 | B0: Naive | B1: Purity/Support | B2: Richer |
|---|---|---|---|
| 准入聚类数 | 20 | 19 | **5** |
| 拒绝聚类数 | 0 | 1 | **15** |
| 基础设施入选 | 12 | 11 | **5** |
| 动态（叉车）入选 | 4 | 4 | **0** |
| 幻影风险（Phantom risk） | 20.0% | 21.1% | **0.0%** |
| 稳定性比率 | 60.0% | 57.9% | **100.0%** |

**关键发现：**

1. **B0→B2：** 维护层消除全部 4 个叉车聚类（幻影风险 20%→0%）和 4 个未分类单 session 聚类，保留全部 5 个通过跨 session 验证的基础设施。这就是准入控制方法的全部价值。

2. **B1 相比纯量准入几乎没有改进。** 仅多拒绝了 1 个低支撑单 session 聚类，仍然准入全部 4 个叉车。叉车聚类具有高纯度（≥0.83）和高支撑度（≥6），仅靠纯度和支撑度无法将其与基础设施区分。

3. **B1 与 B2 之间的差距正是跨 session 证据的贡献：** session_count 和 dynamic_ratio 分离了纯度单独无法区分的内容。

**B1 限制：** B1 是代理基线——观测提取时未记录 Grounding DINO 置信度。label_purity 衡量的是标注一致性而非检测质量。真实置信度基线需要重新运行前端并记录置信度。代理基线已刻意宽松（purity≥0.85 是高门槛，support≥10 是中门槛）以给启发式策略最佳机会——它仍然无法拒绝叉车。

**完整对比输出：** `outputs/torwic_baseline_comparison_results.json`
...
## 附录：按类别留存率与拒绝原因分析（P157）

以下分析按对象类别和拒绝原因分解地图准入决策，为审稿人提供每类对象为何保留/拒绝的完整审计。

![Fig. 14. 按类别留存/拒绝柱状图（P157）。](figures/torwic_per_category_retention_p157.png)

![Fig. 15. 拒绝原因分布（P157）。](figures/torwic_rejection_reason_distribution_p157.png)

![Fig. 16. 类别×拒绝原因热力图（P157）。](figures/torwic_rejection_reason_heatmap_p157.png)
...
*解读：* 基础设施类别的选择性适中：策略仅准入具有多 session 证据和空间多样性的聚类（≥2 sessions, ≥4 frames）。叉车拒绝率 100%，dynamic_contamination 是唯一或主要驱动力。基础设施类别间的留存率差异反映了自然数据约束（某些障碍物/货架仅出现在 1 个 session 中），而非策略的类别偏见。
...
| 拒绝原因 | 触发聚类数 | 占拒绝 (n=15) |
|---|---:|---:|
| **单 Session** (sessions < 2) | 7 | 47% |
| **低帧数** (frames < 4) | 8 | 53% |
| **低支撑度** (observations < 6) | 5 | 33% |
| **动态污染** (ratio > 0.20) | 4 | 27% |
| **标签碎片化** (purity < 0.70) | 1 | 7% |

*解读：* 两个最普遍的拒绝驱动力——单 session 证据和低帧数——是互补的。动态污染集中在叉车类别（全部 4 个叉车聚类）。标签碎片化很少见（1 个聚类），表明开放词汇 pipeline 即使对不可准入的对象也产生内部一致的标签。

### 3. 类别×拒绝原因矩阵

| 类别 | 单 Session | 低帧数 | 低支撑度 | 动态污染 | 标签碎片 |
|---|:---:|:---:|:---:|:---:|:---:|
| **黄色障碍物** | 3 | 0 | 0 | 0 | 0 |
| **工作台** | 2 | 0 | 0 | 0 | 0 |
| **仓库货架** | 1 | 2 | 2 | 0 | 1 |
...
*解读：*
- **黄色障碍物**仅因 session 覆盖不足被拒绝（仅出现在 1 个 session）。
- **工作台**仅因 session 覆盖不足被拒绝（仅出现在 1 个 session）。
- **仓库货架**拒绝原因最多样化：单 session (1)、低帧数 (2)、低支撑度 (2)、标签碎片化 (1)，反映了货架结构检测条件的挑战。
- **叉车**因低帧数（全部 4 个<4 frames）、低支撑度（2 个<6 observations）和动态污染（全部 4 个 ratio≥0.83）被拒绝。

### 4. 证据小结

1. **类别级可解释性已证。** 每个拒绝均可追溯到具体的标准违反和命名类别，支持审稿人的完整审计。
2. **策略无类别偏见。** 同一个五门数值标准应用于所有类别。叉车因违反动态污染门（以及通常同时违反帧数/支撑度门）被拒绝，而非因针对叉车的排除规则。
3. **基础设施拒绝源于证据不足。** 黄色障碍物和工作台被拒绝主要是因为仅出现在 1 个 session——这是回访覆盖有限的自然后果，而非策略缺陷。
4. **策略自验证。** dynamic_ratio 门自然分离基础设施（全部 ratio=0.00）和叉车（全部 ratio≥0.83），零模糊边界案例——无聚类落在中间区间（0.01–0.82）。这种双峰分布是数据自然涌现的，而非策略强制。

**完整分析输出：** `outputs/torwic_category_retention_analysis_p157.json`
