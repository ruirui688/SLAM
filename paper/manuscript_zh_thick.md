...
2. **辅助证据：有界动态掩码前端。** 整合 P217-P240 的数据集、compact UNet、ORB proxy、60 帧 DROID raw-vs-masked smoke、P228 hard gate、P233-P234 失败诊断、P235-P237 soft-boundary candidate 和 P240 mixed self-supervised diagnostics。该线只支持前端模块故事，不支持完整动态 SLAM、导航效果或 independent-label 声明。
...
动态掩码路径训练或应用动态区域 mask，进行后处理、masked RGB 输入或特征区域分析，并报告有界 raw-vs-masked 证据。当前结论是：learned mask 可以抑制预测动态区域中的 ORB 特征，hard black gate 存在边界特征失败模式，而 soft boundary/mean-fill feather candidate 在有界 smoke 中修复了该回归窗口，并扩展到四个额外短窗口，同时 DROID-SLAM 轨迹 delta 在有界 gate 中保持中性。P240 增加 mixed self-supervised consistency 证据，但这不是 independent labels。
...
### 六.D. P228-P240 Confidence/Coverage Gate、Soft-Boundary Candidate 与 Consistency Diagnostics
...
P237 在不重训、且不使用目标窗口标签的前提下，将同一候选扩展到四个额外短窗口。三个窗口来自 Oct. 12, 2022 Aisle_CW 且不与 120-179、480-539、840-899 重叠，另一个来自 nearby Oct. 12, 2022 Aisle_CCW。候选在四个窗口均降低 ORB predicted-region keypoints：Aisle_CW 240-299 为 218 -> 41，Aisle_CW 660-719 为 41 -> 0，Aisle_CW 960-1019 为 27 -> 0，Aisle_CCW 240-299 为 81 -> 4。四个窗口 total ORB keypoint delta 均为 0。四个 16-frame DROID gates 均为 neutral，代表性 Aisle_CW 240-299 的 60-frame gate 也为 neutral（dAPE +0.000201，dRPE +0.000003）。

P239 随后准备 independent dynamic-label mini packet 供后续人工审阅，但没有收集标签，也没有验证候选。P240 对同一候选在五个 60 帧窗口上做 self-supervised consistency diagnostics：840-899、Aisle_CW 240-299、Aisle_CCW 240-299、480-539 和 120-179。结果状态是 `mixed_self_supervised_support`。其中三个窗口通过全部 support flags：`aisle_cw_0240_0299`、`aisle_ccw_0240_0299` 和 `aisle_cw_0480_0539`。840-899 和 120-179 保持 temporal coherence，并且 soft region 和 boundary band 内没有 ORB inflation，但没有达到 motion/frame-difference enrichment 阈值。hard-mask 对照仍支持 P234 hard-edge failure diagnosis：840-899 上 hard variants 的 region ORB deltas 为 [1070, 1300]，boundary-band ORB deltas 为 [2912, 5377]。

正确解释应保持很窄：hard black confidence/coverage gating 有已记录的 boundary-feature failure mode，P235/P237/P240 soft-boundary candidate 在多个短窗口上获得 expanded but mixed bounded support。P240 是 diagnostic self-supervision，不是 independent human-label validation。它不是 full dynamic-SLAM benchmark、navigation gain、independent-label validation 或 learned map-admission result。
...
| P237 | 三个新 Aisle_CW 窗口和一个 Aisle_CCW 窗口上的 soft-boundary candidate 验证 | 扩展的有界前端模块支持；不改变声明边界。 |
| P239-P240 | independent-label packet 准备和 self-supervised consistency diagnostics | P239 没有收集标签；P240 是 mixed diagnostic support，不解锁 P195。 |
...
| P237 soft-boundary multi-window | 新窗口：Aisle_CW 240-299 ORB 218 -> 41，Aisle_CW 660-719 ORB 41 -> 0，Aisle_CW 960-1019 ORB 27 -> 0，Aisle_CCW 240-299 ORB 81 -> 4；均 total delta 0；DROID 16f 4/4 neutral；代表性 60f neutral | 覆盖更多短窗口和一个 nearby sequence 的 expanded bounded frontend-module support；不声明 full benchmark 或 navigation。 |
| P240 self-supervised consistency | status `mixed_self_supervised_support`；3/5 窗口通过全部 support flags；840-899 和 120-179 有 temporal/ORB 支持但未通过 motion/frame-difference enrichment；hard-mask 对照显示 region ORB deltas [1070, 1300] 和 boundary-band deltas [2912, 5377] | 对 failure-driven soft-boundary design 的 mixed diagnostic support；不是 independent labels，不解锁 P195。 |
...
| ORB predicted/gated-region suppression | proxy 支持，hard-boundary 失败模式已记录，P235/P237 候选有扩展的有界支持 |
| Self-supervised soft-boundary consistency | 仅 mixed diagnostic support：3/5 窗口通过全部 flags，840-899 和 120-179 未通过 motion/frame-difference enrichment 阈值 |

正确声明是有界前端 smoke：learned、hard-gated 和 soft-boundary masks 可以在短工业窗口上生成，通过 DROID-SLAM 执行，并用于诊断预测动态区域中的 ORB 特征行为。hard black gate 不能升级为 multi-window support；P235/P237/P240 soft-boundary candidate 仍需更广序列覆盖和独立标签。
...
6. P225-P240 是有界 smoke 和 diagnostic tests；Oct. 12, 2022 Aisle_CW/Aisle_CCW 证据是短窗口受限前端检查。P233-P234 说明 hard black gating 可能因边界关键点失败，P235-P240 仍是候选 soft-boundary 证据而不是宽泛 benchmark。P240 是 mixed self-supervised support，不是 independent-label validation。
...
本文还整合了 P217-P240 动态掩码前端证据。compact learned mask、60 帧 learned-mask package、DROID raw-vs-masked reproduction、P228 confidence/coverage gate、P233-P234 hard-boundary 失败诊断、P235-P237 soft-boundary candidate 和 P240 mixed self-supervised consistency check 形成一个有界前端故事：hard black gate 在原始窗口能降低 ORB 数量，但在回归窗口会制造边界关键点；mean-fill feathering 在有界 smoke 中移除了该观测回归，并在四个额外短窗口中保持 ORB-positive。P240 只把该设计支撑为 mixed diagnostic module result，不是 independent ground truth。该结果增强了论文的动态场景 SLAM 语境，但不越界为宽泛评测、导航、独立 GT 或 learned map-admission 声明。
...
**表 5. 动态掩码前端证据。** P217-P240 数据集、模型、ORB proxy、learned-mask sequence、DROID baseline、hard confidence/coverage gate、multi-window failure 证据、扩展 soft-boundary candidate validation 和 mixed self-supervised consistency diagnostics。

**表 6. P225-P240 有界 DROID smoke 和 diagnostics。** P227 learned-mask baseline、P228 confidence/coverage-gated module、P233-P234 hard-gate failure evidence、P235-P237 soft-boundary candidate evidence 和 P240 self-supervised consistency evidence。按轨迹中性和 ORB proxy diagnostics 报告，不作为 navigation、independent-label 或 full-benchmark claim。

**图 1-16.** 图像路径沿用英文稿：`paper/figures/torwic_*`。P225-P241 证据见 `paper/export/temporal_masked_sequence_p225.md`、`paper/export/p225_baseline_reproduction_p227.md`、`paper/export/confidence_gated_mask_module_p228.md`、`paper/export/p228_paper_story_results.md`、`paper/export/gated_mask_multi_window_p233.md`、`paper/export/gated_mask_failure_sweep_p234.md`、`paper/export/soft_boundary_mask_p235.md`、`paper/export/soft_boundary_multiwindow_p237.md`、`paper/export/soft_boundary_consistency_p240.md`、`paper/export/thick_results_lock_p241.md`。
