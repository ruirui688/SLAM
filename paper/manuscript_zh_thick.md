...
本文同时整合后续动态掩码前端证据。P217 数据集包含 237 行，156/51/30 划分，零帧重叠，正像素率 0.374176。P218 compact UNet 的验证 IoU/F1 为 0.671304/0.803329，测试 IoU/F1 为 0.578580/0.733038。P219 held-out precision/recall/F1/IoU 为 0.556007/0.789669/0.604636/0.443210。P220 中 GT 动态区域 ORB 关键点从 4795 降到 2192，减少 54.2857%。P225-P228 构建 60 帧 Oct. 12, 2022 Aisle_CW raw-vs-masked 序列，复现中性的 DROID-SLAM baseline，并加入 confidence/coverage-gated mask module。P228 使用阈值 0.50、1 px 膨胀、128 px 最小连通域、0.22/0.18 最大/目标覆盖，平均覆盖 14.127053%；60 帧 raw/masked APE 为 0.088496/0.084705，RPE 为 0.076145/0.076224，ORB gated-region 关键点从 8969 降到 5073。该结果是可用的前端故事种子，不是完整 benchmark、导航收益、独立动态分割真值或 learned persistent-map admission 结论。P195 仍为 BLOCKED。
...
2. **辅助证据：有界动态掩码前端。** 整合 P217-P228 的数据集、compact UNet、ORB proxy、60 帧 DROID raw-vs-masked smoke 和 P228 gated module。该线只支持前端模块故事，不支持完整动态 SLAM 或导航收益声明。

本文不声称解决完整 lifelong SLAM、稠密动态重建、独立动态分割真值、下游导航收益或 learned persistent-map admission。
...
P228 60 帧 raw/masked APE 为 0.088496/0.084705，RPE 为 0.076145/0.076224，ORB gated-region 关键点 8969 -> 5073。相比 P227，P228 轨迹 smoke 仍中性，但预测区域内特征抑制更集中，是当前可用的动态掩码前端故事种子。
...
| P225-P228 | 60 帧 Oct. 12, 2022 Aisle_CW learned/gated mask DROID smoke | 有界前端故事种子，不是 benchmark。 |
...
| P228 gated module | APE 0.088496 -> 0.084705；RPE 0.076145 -> 0.076224；ORB 8969 -> 5073；mean coverage 14.127053% | 有界前端故事种子；预测区域抑制更强，轨迹中性。 |
...
6. P225-P228 是有界 smoke；60 帧 DROID 轨迹结果中性，不是 benchmark。
...
本文还整合了 P217-P228 动态掩码前端证据。compact learned mask、60 帧 learned-mask package、DROID raw-vs-masked reproduction 和 P228 confidence/coverage gate 形成一个有界但可用的前端故事：预测动态区域内特征抑制增强，而轨迹 smoke 保持中性。该结果增强了论文的动态场景 SLAM 语境，但不越界为 benchmark、导航收益、独立 GT 或 learned map-admission 声明。
