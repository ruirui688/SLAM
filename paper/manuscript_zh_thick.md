...
1. **不是完整的终身SLAM后端。** 维护层（§V）是感知与地图之间的中间过滤器。它不进行回环闭合、位姿优化或地图大小管理——这些是完整SLAM系统[1][3][5]提供的能力。当前 bounded DROID-SLAM raw-vs-masked 运行已验证后端执行/evo 路径（§七.E），包括 64 帧 global BA 运行，但完整轨迹后端评估仍为未来工作。
