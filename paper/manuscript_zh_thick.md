...
过道阶梯（TorWIC_s1_d45，来自POV-SLAM[6]）证明维护层保留稳定的基础设施物体，同时拒绝动态污染。全部三个协议使用相同的准入标准（§VI.C）：
...
1. **叉车被以高置信度检测到。** 检测器工作正常——它识别了叉车。在我们的过道协议（§VII.A）中，叉车类簇的平均检测置信度与保留的基础设施物体相当，但全部16个被准入标准（§V.E）正确拒绝为动态污染。高检测置信度不区分"这是一个真实的叉车"和"这是一个值得进入地图的静态地标"。

2. **低置信度的稳定物体可能是最有价值的。** 一个在五个会话中以中等置信度被检测到的护栏对地图复用比一个以高置信度被检测到的叉车更有价值。置信度阈值为检测优化[1]，而非为地图准入优化。我们的证据阶梯（§VII.B）显示，保留物体（工作台、仓储货架、护栏）即使在逐帧置信度波动的情况下，也一致通过多会话和标签一致性标准。
...
1. **不是完整的终身SLAM后端。** 维护层（§V）是感知与地图之间的中间过滤器。它不进行回环闭合、位姿优化或地图大小管理——这些是完整SLAM系统[1][3][5]提供的能力。与完整SLAM管线的集成仍为未来工作。
...
6. **后端模型引用暂缓。** Grounding DINO、SAM2和OpenCLIP在观察提取管线（§V.A）中作为黑盒组件使用。正式引用这些模型需要用户偏好——它们将扩展当前[1]–[6]的参考文献。其角色在§V.A和P125审计中已明确记录，以供读者复现。
