...
2. **辅助证据：有界动态掩码前端。** 整合 P217-P237 的数据集、compact UNet、ORB proxy、60 帧 DROID raw-vs-masked smoke、P228 hard gate、P233-P234 失败诊断和 P235-P237 soft-boundary candidate。该线只支持前端模块故事，不支持完整动态 SLAM 或导航效果声明。
...
动态掩码路径训练或应用动态区域 mask，进行后处理、masked RGB 输入或特征区域分析，并报告有界 raw-vs-masked 证据。当前结论是：learned mask 可以抑制预测动态区域中的 ORB 特征，hard black gate 存在边界特征失败模式，而 soft boundary/mean-fill feather candidate 在有界 smoke 中修复了该回归窗口，并扩展到四个额外短窗口，同时 DROID-SLAM 轨迹 delta 在有界 gate 中保持中性。
...
### 六.D. P228-P237 Confidence/Coverage Gate 与 Soft-Boundary Candidate
...
正确解释应保持很窄：hard black confidence/coverage gating 有已记录的 boundary-feature failure mode，P235/P237 soft-boundary candidate 在多个短窗口上获得 expanded bounded support。它不是 full dynamic-SLAM benchmark、navigation gain、independent-label validation 或 learned map-admission result。
...
正确声明是有界前端 smoke：learned、hard-gated 和 soft-boundary masks 可以在短工业窗口上生成，通过 DROID-SLAM 执行，并用于诊断预测动态区域中的 ORB 特征行为。hard black gate 不能升级为 multi-window support；P235/P237 soft-boundary candidate 仍需更广序列覆盖和独立标签。
...
6. P225-P237 是有界 smoke；Oct. 12, 2022 Aisle_CW/Aisle_CCW 证据是短窗口受限前端检查。P233-P234 说明 hard black gating 可能因边界关键点失败，P235-P237 仍是候选 soft-boundary 证据而不是宽泛 benchmark。
...
本文还整合了 P217-P237 动态掩码前端证据。compact learned mask、60 帧 learned-mask package、DROID raw-vs-masked reproduction、P228 confidence/coverage gate、P233-P234 hard-boundary 失败诊断和 P235-P237 soft-boundary candidate 形成一个有界前端故事：hard black gate 在原始窗口能降低 ORB 数量，但在回归窗口会制造边界关键点；mean-fill feathering 在有界 smoke 中移除了该观测回归，并在四个额外短窗口中保持 ORB-positive。该结果增强了论文的动态场景 SLAM 语境，但不越界为宽泛评测、导航、独立 GT 或 learned map-admission 声明。
...
**表 5. 动态掩码前端证据。** P217-P237 数据集、模型、ORB proxy、learned-mask sequence、DROID baseline、hard confidence/coverage gate、multi-window failure 证据和扩展 soft-boundary candidate validation。

**表 6. P225-P237 有界 DROID smoke。** P227 learned-mask baseline、P228 confidence/coverage-gated module、P233-P234 hard-gate failure evidence 和 P235-P237 soft-boundary candidate evidence。按轨迹中性和 ORB proxy diagnostics 报告，不作为 navigation 或 full-benchmark claim。

**图 1-16.** 图像路径沿用英文稿：`paper/figures/torwic_*`。P225-P238 证据见 `paper/export/temporal_masked_sequence_p225.md`、`paper/export/p225_baseline_reproduction_p227.md`、`paper/export/confidence_gated_mask_module_p228.md`、`paper/export/p228_paper_story_results.md`、`paper/export/gated_mask_multi_window_p233.md`、`paper/export/gated_mask_failure_sweep_p234.md`、`paper/export/soft_boundary_mask_p235.md`、`paper/export/soft_boundary_multiwindow_p237.md`、`paper/export/thick_results_lock_p238.md`。
