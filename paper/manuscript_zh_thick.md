...
1. **叉车被以高置信度检测到。** 检测器工作正常——它识别了叉车。在我们的过道协议（§VII.A）中，叉车类簇的平均检测置信度与保留的基础设施物体相当，但全部被准入标准（§V.E, §VII.D）正确拒绝为动态污染。高检测置信度不区分"这是一个真实的叉车"和"这是一个值得进入地图的静态地标"。我们的基线对比（§VII.G.2）直接量化了这一点：近似置信度过滤的纯度/支持度启发式（B1）仍然准入全部4个叉车（幻影风险21.1%），而完整跨会话策略（B2）将其全部消除（0.0%）。
