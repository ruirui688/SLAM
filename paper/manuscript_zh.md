...
6. 给出当前 dataset-mask-supervised 动态/非静态前端训练和 raw-vs-masked baseline 复现证据，包括 P218 compact UNet、P227 DROID baseline、以及 P228 confidence/coverage-gated mask 模块。
...
当前学习型动态 mask 前端与地图准入层保持分离。P217 使用 TorWIC/AnnotatedSemanticSet 中 dataset-provided semantic/indexed masks 构造二值 dynamic/non-static 训练目标；P218 训练 compact UNet-style binary mask model；P225-P228 则把该路线接入 timestamped raw-vs-masked 序列、DROID/evo 复现路径和一个小的 confidence/coverage-gated 后处理模块。该模块只控制前端 mask 的置信度、连通域、膨胀和覆盖率，不生成独立地图准入标签。
...
表 3 固化当前学习型 mask 前端的 baseline 和模块结果。P227 是 P225 60 帧 learned-mask 序列上的 DROID baseline 复现；P228 只改变后网络 mask 后处理模块，使用相同 DROID/evo 和 ORB proxy 路径。

| 前端变体 | 帧数 | APE RMSE Raw -> Masked (m) | RPE RMSE Raw -> Masked (m) | ORB region keypoints Raw -> Masked | 解释 |
|---|---:|---:|---:|---:|---|
| P227 learned-mask baseline | 60 | 0.088504 -> 0.084529 | 0.076145 -> 0.076226 | 21030 -> 18617 | baseline 复现通过，轨迹 smoke 中性 |
| P228 confidence/coverage-gated module | 60 | 0.088496 -> 0.084705 | 0.076145 -> 0.076224 | 8969 -> 5073 | 轨迹基本中性，gated region 特征抑制更集中 |

该表应保守解读。ORB region 使用 predicted-mask-region proxy，不是独立动态 GT；DROID 数字来自单个 60 帧 bounded smoke，不是完整 benchmark。可以使用的论文表述是：P228 在不破坏 P227 baseline 轨迹 smoke 的前提下，提高了前端 gated region 内的 ORB 特征抑制。
...
P228 模块的价值同样应按“前端过滤控制”来解释，而不是按“轨迹显著提升”来解释。它说明：在同一 P225 序列、同一 DROID/evo 评估路径下，confidence/coverage-gated mask 可以保持 trajectory smoke 基本中性，同时更集中地压制预测动态区域内的 ORB features。这为后续更大范围 benchmark 和更强网络模块提供了故事种子。
...
P225-P228 的 DROID/ORB 数字也是 bounded local smoke。它们使用单个 60 帧 TorWIC `Aisle_CW` 窗口、predicted-mask-region ORB proxy 和现有 DROID runtime，可用于固定 baseline 和模块故事，但不能扩展为广义轨迹、导航或部署机器人收益。
